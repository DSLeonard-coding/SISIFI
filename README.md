...
The compiler will warn about about the shadowing.  It's best to disable the warning locally with pragmas, becuse you do need
to notice the warning elsewhere for concrete shadowing.  Clang-tidy (not the compiler) also wanrs about default arguments for virtual methods. I find this warning silly in general, but for our tags the default value is irrelevant anyway, so we can ignore it.
...
## SISIFI fluency with C++29 Code injection:  Removing the boilerplate  
Code injection is comming in, maybe, C++29 and as I understand we could annotate the interface methods as fluent, and get the Interface implementation built for free.  I'm sure this will happen quickly with some fluent schemes, but even with automation, ths may be a more direct solution than others.


##  Why has this has been hard to see and is it safe?
It has been hard to see because C++ inheritance concepts just traditionally aren't meant to limit method inheritance to one level, and it's not the normal way of thinking.  Mostly that it's it, although it's compounded by the fact that hiding is both by syntax and history a side-effect, not a feature, and something usually warned about, not leveraged.  It literally produces a compiler warning, -Woverload-virtual. 

Still,  A danger of hiding is that there is potential for surprises. In this pattern, the tag is added to abstract classes at the interface level.  It must be inherited at the interface implementation to satisfy the contract, and the base class must define a method to satisfy the delegation, so that's all pretty safe.  Probably the worst danger is forgetting to mark the next level method as virtual, if you need continued polymorphism. Normally it remains virtual.  Here, by design, it's a new method, so it can be virtual or not.  This adds flexibility, but a little risk, none without compiler warnings though.
