# Subscribable-Inheritance Shadow Interfaces (SISI) and the SISI Fluent Idiom (SISIFI)  
*A Novel C#-equivalent, zero-overhead polymorphic fluent design for C++, by D.S Leonard.*

(Now, tested, code coming soon.  Should move to its own page)
...
You have fully polymorphic IBuilder views. This creates a little boilerplate, just as in C#.  It's less syntactically idiomatic than in C#, but it creates a very straightforward hierarchical solution that is expandable. In [DLG4::VolumeBuilders](dsleonard-coding.github-io/VolumeBuilders), it could include VolumeBuilder and StructureBuilder naturally, and eliminates a lot of conversion ctors and view links. 

If we didn't mind calling DoWork() on the concrete  type and IDoWork() on the interface type, there would never have even been an issue.  IDoWork could delegate polymopphically to Concrete::DoWork and that's it.  And honestly, that's not even a bad option among bad options.   The only problem was how to make the names the same but the return types different.  The traditional wisdom I could find has been that you cannot do that with inheritance.  It's hard to find evidence that anyone ever thought "so don't inherit."  
...
Still,  A danger of hiding is that there is potential for surprises. In this pattern the tag is added to abstract classes at the interface level.  It must be inherited at the interface implementation to satisfy the contract, and the pattern is that you delegate from there to the next inheritance level.  So if the next level omits the tag, they can change their return type.  If you accidentally include the tag, you cannot. Probably the worst danger is forgetting to mark the next level method as virtual. So, it can be broken, but it's not invisible, and if you re-shadow it without marking it virtual it will at least generate a compiler warning.  
...
It _could_ even be worth re-working VolumeBuilders to use this pattern.  It shouldn't require changes to the user code, in principle. 
